...
def display():
  while True:
    choice = input(
        "What service you would like to use? (1-Tasks, 2-Finances): ")
    if choice not in ["1", "2"]:
      print("Invalid Option. Please try again.")
    else:
      break
...
def option_4_finances(user_id):
  ...
  while True:
    expense_name = input("Name of expense: ")
    expense = db.get_expenses_by_name(expense_name=expense_name,
                                      user_id=user_id)
...
def option_5_finances(user_id):
  ...
  while True:
    expense_name = input(
        "Type the name of the expense you would like to update: ")
    expense = db.get_expenses_by_name(expense_name=expense_name,
                                      user_id=user_id)
